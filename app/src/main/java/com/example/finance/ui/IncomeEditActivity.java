...
public class IncomeEditActivity extends Activity {
        ...
        try {
            if (c.moveToFirst()) {
                ...
                if (!"INCOME".equals(type)) {
                    Toast.makeText(this, "Bản ghi không phải THU", Toast.LENGTH_SHORT).show();
                    finish();
                    return;
                }
    ...
    private void onSave() {
        ...
        if (editingId > 0) {
            ...
            db.insertTransaction(uid, catId, "INCOME", amount, date, note);
            Toast.makeText(this, getString(R.string.msg_added_income), Toast.LENGTH_SHORT).show();
        }
